...
is what asserts that none of them failed; see the comment at the top of that
file before adding a job to it. The editor, live preview, anonymous drafts (autosaved to `localStorage`),
...
```bash
git clone https://github.com/Poetic-Poems/poetic-fiddle.git <scratch-dir>/poetic-fiddle
```
...
can rebase. Commit, push the feature branch, and open the pull request from that clone; delete the clone
