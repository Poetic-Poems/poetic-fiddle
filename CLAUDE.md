...
- **Single source of truth.** Fiddle renders poems with a browser-safe renderer
  **exported by the `poetic` repo**, not a copy. Do not fork or re-implement the
  ...
  module arrives as a **tag-pinned release-tarball dependency** on the
  `poetic` repo (`docs/IMPLEMENTATION-PLAN.md` §6.1); bumping it is a
  deliberate `package.json` edit, never a silent float.
